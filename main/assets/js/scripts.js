...
// Usage: codyhouse.co/license
(function() {
  var appUi = document.getElementsByClassName('js-backend-ui');
  if(appUi.length > 0) {
    var appMenuBtn = appUi[0].getElementsByClassName('js-backend-ui__menu-btn');
    if(appMenuBtn.length < 1) return;
    var appExpandedClass = 'backend-ui--nav-expanded';
    ...
    window.addEventListener('keyup', function(event){
      ...
      if( (event.keyCode && event.keyCode == 9) || (event.key && event.key.toLowerCase() == 'tab' )) {
        ...
        if(appMenuBtn[0].getAttribute('aria-expanded') == 'true' && isVisible(appMenuBtn[0]) && !document.activeElement.closest('.js-backend-ui__nav')) appMenuBtn[0].click();
      }
    });
    ...
    function getMenuFirstFocusable() {
      var mobileNav = appUi[0].getElementsByClassName('js-backend-ui__nav');
